...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // 通知父组件数据变化，传递当前页数据和总数
        onDataChange?.(dataArray, response.data.total || 0);
      } else {
  ...
  }, [sessionId, currentPage, pageSize, allFilters, sorts, onDataChange]);
  ...
  const exportData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        link.setAttribute(
          "download",
          `data_export_${new Date().getTime()}.csv`
        );
  ...
  // 构建表格列
  const columns: ColumnsType<any> = (fields || []).map((field: FieldInfo) => {
    return {
      title: field.label,
      ...
      {showToolbar && (
        ...
        >
          <Space>
            ...
            {allFilters.length > 0 && (
              <Tag color="blue">{allFilters.length} 个筛选条件</Tag>
            )}
            {selectedRowKeys.length > 0 && (
              <Tag color="green">已选择 {selectedRowKeys.length} 行</Tag>
            )}
          </Space>
