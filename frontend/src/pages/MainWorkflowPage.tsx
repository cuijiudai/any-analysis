  ...
  const loadFetchedData = async (sessionId: string) => {
    try {
      ...
      const dataResponse = await api.get(
        `/data-fetch/data/${sessionId}?page=1&pageSize=${pageSize}`
      );
        ...
        <div>
          {fetchStatus && (
            <Card size="small" style={{ marginBottom: 16 }}>
              <Space>
                <span>
                  拉取状态: <strong style={{ color: "#52c41a" }}>完成</strong>
                </span>
                <span>
                  总记录数: <strong>{fetchStatus.totalRecords}</strong>
                </span>
                {fetchStatus.pagesProcessed && (
                  <span>
                    处理页数: <strong>{fetchStatus.pagesProcessed}</strong>
                  </span>
                )}
              </Space>
            </Card>
